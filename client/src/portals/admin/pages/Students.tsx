...
import { Search, Plus, Edit3, Trash2, ChevronLeft, ChevronRight, X, RefreshCw, GraduationCap, Bus, Footprints } from "lucide-react";
...
function StudentModal({ open, onClose, initial, sections, busRoutes, onSave }: {
  ...
  const blank = { firstName: "", lastName: "", email: "", phone: "", gender: "MALE", dateOfBirth: "", address: "", admissionNo: "", sectionId: "", rollNo: "", transportMode: "WALKING", busRouteId: "", avatar: "", class10Marks: "", entranceMarks: "", stream: "" };
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="pt-2 border-t border-gray-100">
            ...
            <div className="mt-3">
              <label className="block text-xs font-medium text-gray-600 mb-1.5">Stream / Program <span className="text-gray-300">(for +2 / Bachelor / Master)</span></label>
  ...
  return (
    <>
                  ...
                  <tr key={s.id} className="border-t border-gray-50 hover:bg-gray-50/60 group cursor-pointer" onClick={() => navigate(`/admin/students/${s.id}`)}>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => { setEditStudent(s); setModal(true); }} className="p-1.5 text-gray-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors"><Edit3 className="w-3.5 h-3.5" /></button>
                        <button onClick={() => setDeleteId(s.id)} className="p-1.5 text-gray-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <StudentModal open={modal} onClose={() => { setModal(false); setEditStudent(null); }} initial={editStudent}
        ...
        onSave={async (d) => {
          if (editStudent) await api.admin.updateStudent(editStudent.id, d);
          else await api.admin.createStudent(d);
          reload();
        }} />
    </>
  );
