...
export default function StudentExams() {
  const { data, loading } = useList<any[]>(() => api.student.exams());
  ...
  return (
    <div className="p-6 space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Upcoming and past exam schedules</p>
      </div>
              ...
              <div key={exam.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                ...
                {isExpanded && subjects.length > 0 && (
                  <div className="border-t border-gray-100 bg-gray-50/50 px-5 py-4">
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Exam Routine</p>
                        ...
                        <tbody>
                          {subjects.map((s: any) => (
                            <tr key={s.id} className="border-t border-gray-50">
                              ...
                              <td className="px-4 py-2 font-semibold text-gray-900">{s.subject?.name ?? s.subjectName ?? "—"}</td>
                ...
                {isExpanded && subjects.length === 0 && (
                  <div className="border-t border-gray-100 px-5 py-3">
                    <p className="text-xs text-gray-400">No routine published for this exam yet.</p>
                  </div>
                )}
              </div>
            );
