...
export default function Notices() {
  ...
  const load = async () => {
    ...
    const [n, notifs] = await Promise.all([
      api.student.notices().catch(() => []),
      api.student.notifications().catch(() => []),
    ]);
    setNotices(n ?? []);
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-5">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">School notices, announcements, and updates for students</p>
        </div>
